...
namespace HotelListing.API.CoreRepository
{
    public class AuthMAnager : IAuthManager
    {
        ...
        {
            _logger.LogInformation($"Looking for User with the Email {loginDto.Email}");
            _user = await _userManager.FindByEmailAsync(loginDto.Email);
            bool isValidUser = await _userManager.CheckPasswordAsync(_user, loginDto.Password);

            if (_user == null || isValidUser == false)
            {
                _logger.LogWarning($"There is no User with that email {loginDto.Email}");
                return null;
            }
            var token = await GenerateToken();

            _logger.LogInformation($"Token Was Successfully Generated for User {loginDto.Email} | " +
                $"Token {token}");
